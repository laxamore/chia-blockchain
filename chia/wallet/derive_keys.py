...
def _derive_path_unhardened_pk(pk: G1Element, path: List[int]) -> G1Element:
    for index in path:
...
def master_pk_to_wallet_pk_unhardened_intermediate(master: G1Element) -> G1Element:
    return _derive_path_unhardened_pk(master, [12381, 8444, 2])
...
def master_pk_to_wallet_pk_unhardened(master: G1Element, index: uint32) -> G1Element:
    ...
    return _derive_path_unhardened_pk(intermediate, [index])
